...
static unsigned int mmcqd_rq_count[ID_CNT]={0};
	...
	if (unlikely(dumpMMCqd())) {	
		AddStorageTrace(STORAGE_LOGGER_MSG_ISSUE_RQ,mmc_blk_issue_rq,(rq_data_dir(req) == WRITE),
			...
			if(t_period >= (unsigned long long )PRT_TIME_PERIOD)
			{
				...
				{
					xlog_printk(ANDROID_LOG_INFO, "BLOCK_TAG", "mmcqd pid:%d, timeline %lld, duty %lld, period %lld, < 1%% I/O time occupied, rq_count=%d \n", mmcqd[idx], mmcqd_last_t2[idx], t_usage, t_period, mmcqd_rq_count[idx]);
				}
				else
				{
					t_percent =((unsigned int)t_usage)/((unsigned int)t_period/100);
					xlog_printk(ANDROID_LOG_INFO, "BLOCK_TAG", "mmcqd pid:%d, timeline %lld, duty %lld, period %lld, %d%% I/O time occupied, rq_count=%d \n", mmcqd[idx], mmcqd_last_t2[idx], t_usage, t_period, t_percent, mmcqd_rq_count[idx]);					
				}
				if(mmcqd_t_usage_wr[idx])
				{
					perf_meter = (mmcqd_rq_size_wr[idx]*10)/((unsigned int)mmcqd_t_usage_wr[idx]/100000); //kb/s
					xlog_printk(ANDROID_LOG_INFO, "BLOCK_TAG", "mmcqd pid:%d, WRITE size: %d bytes, delta time:%lld, throughput: %d kB/s\n", mmcqd[idx], mmcqd_rq_size_wr[idx], mmcqd_t_usage_wr[idx], perf_meter);
				}
				if(mmcqd_t_usage_rd[idx])
				{
					perf_meter = (mmcqd_rq_size_rd[idx]*10)/((unsigned int)mmcqd_t_usage_rd[idx]/100000);//kb/s
					xlog_printk(ANDROID_LOG_INFO, "BLOCK_TAG", "mmcqd pid:%d, READ size: %d bytes, delta time:%lld, throughput: %d kB/s \n", mmcqd[idx], mmcqd_rq_size_rd[idx], mmcqd_t_usage_rd[idx], perf_meter);  					
				}
				mmcqd_tag_t1[idx]=time1;
			...
			if(rq_byte==0)
				xlog_printk(ANDROID_LOG_INFO, "BLOCK_TAG", "mmcqd pid:%d. rq_byte = 0 \n", mmcqd[idx]);			
#endif
