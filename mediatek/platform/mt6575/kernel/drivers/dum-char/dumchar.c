...
extern int simple_mt6575_sd_ioctl_multi_rw(struct msdc_ioctl* msdc_ctl);
						...
						u32 totalsize, int transtype, Region part)
{
	...
	result = simple_mt6575_sd_ioctl_multi_rw(&cmd);
...
static long dumchar_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if(dev->type == NAND){
		...
		case MEMERASE:
			...
			ebuf  = kmalloc(count, GFP_KERNEL);
			if (!ebuf) {
				printk("DumChar: malloc ebuf buffer fail\n");
				return -ENOMEM;
			}
			memset(ebuf,0xFF, count);
			...
			/*	for(i=0;i<(count+MAX_SD_BUFFER-1)/MAX_SD_BUFFER;i++){
				...
				result = vfs_write(fo->act_filp,ebuf,count,&addr);
				break;
			case BOOT_1:			
				for(i=0;i<(count+MAX_SD_BUFFER-1)/MAX_SD_BUFFER;i++){		
...
#ifdef MTK_EMMC_SUPPORT
	sd_upgrade_proc = create_proc_entry("sd_upgrade", 0666, NULL);
	if (sd_upgrade_proc){
