...
void fgauge_precharge_uninit(void)
{
    ...
    {
        profile_p = fgauge_get_profile(TEMPERATURE_T);
        memset(profile_p_r_table, 0, sizeof(battery_profile_t2));
...
#else
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[get_dynamic_period] no use\r\n");
#endif
...
#else
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC] MT6575 FGADC driver probe!! \n" );
...
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC] MT6575 FGADC driver remove!! \n" );
...
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC] MT6575 FGADC driver shutdown!! \n" );
}
...
#if defined(CONFIG_POWER_EXT)
//#if 0
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_suspend] EVB !!\n");
	...
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_suspend] TODO !!\n");

	#if 0
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_suspend] MT6573 FGADC driver suspend!!\n");
	...
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_suspend] gRTC_time_suspend=%d, gFG_capacity_before=%d\n",
...
#if defined(CONFIG_POWER_EXT)
//#if 0
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] EVB !!\n");

#else

	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] TODO !!\n");
	...
	{
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] MT6573 FGADC driver resume, gFG_DOD0=%d!!\n", gFG_DOD0);
		...
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] FGADC_Reset some SW_Parameter \r\n");
		gFG_columb = 0;
		gFG_SW_CoulombCounter = 0;
		Temp_Reg = 0x18F0;
		...
		if(gRTC_time_offset > gFG_RTC_time_MAX)
		{
			...
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] %d,%d,%d,%d\n", gFG_current, gFG_voltage, gFG_capacity_by_v, bat_volt_check_point);
			...
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] Sleep Time(%d) > %d, gFG_capacity_by_v=%d, bat_volt_check_point=%d, gFG_DOD0=%d, gFG_DOD1=%d, gFG_percent_resume=%d\n",
				gRTC_time_offset, gFG_RTC_time_MAX, gFG_capacity_by_v, bat_volt_check_point, gFG_DOD0, gFG_DOD1, gFG_percent_resume);
		}
		else
		{
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] Need update DOD0(%d) by bat_volt_check_point(%d)\n",
			...
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] Sleep Time(%d) <= %d, gFG_capacity_by_v=%d, bat_volt_check_point=%d, gFG_DOD0=%d, gFG_DOD1=%d\n",
				gRTC_time_offset, gFG_RTC_time_MAX, gFG_capacity_by_v, bat_volt_check_point, gFG_DOD0, gFG_DOD1);
		}

		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] gRTC_time_suspend=%d, gRTC_time_resume=%d, gFG_capacity_before=%d, gFG_capacity_after=%d, temp_RTC=%d, temp_FG=%d\n",
			gRTC_time_suspend, gRTC_time_resume, gFG_capacity_before, gFG_capacity_after, temp_RTC, temp_FG);

		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC_RESUME] %d,%d,%d,%d,%d,gFG_temp=%d,gFG_DOD1_return=%d,val_car=%d\n",
...
static int __init mt6575_fgadc_init(void)
{
	...
	if (ret) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****[mt6575_fgadc_driver] Unable to device register(%d)\n", ret);
		return ret;
	}

	ret = platform_driver_register(&mt6575_fgadc_driver);
	if (ret) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****[mt6575_fgadc_driver] Unable to register driver (%d)\n", ret);
		return ret;
	}

	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****[mt6575_fgadc_driver] Initialization : DONE \n");
