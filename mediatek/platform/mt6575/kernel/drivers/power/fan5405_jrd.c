...
static int8_t fan5405_disable_charging(struct fan5405_struct *fan_struct, uint8_t disable){
	...
	mt_set_gpio_mode(GPIO30, 0);	/*mode 0 as gpio*/
        mt_set_gpio_dir(GPIO30, GPIO_DIR_OUT);		/*griffin:GPIO30(DPIDE); martell:GPIO202 */
        mt_set_gpio_out(GPIO30, (disable == 0)? GPIO_OUT_ZERO : GPIO_OUT_ONE); /*disable == 0: charging controlled by i2c line */
}
...
static int8_t fan5405_check_chgstat_pin(struct fan5405_struct *fan_struct){
	...
	mt_set_gpio_mode(GPIO21, 0);	/*mode 0 as gpio*/
        mt_set_gpio_dir(GPIO21, GPIO_DIR_IN);  /*griffin:GPIO21(DPIVSYNC); martell:GPIO200; get STAT pin status*/
        ret = mt_get_gpio_in(GPIO21);		/**/
...
static int8_t fan5405_set_otg_pin(struct fan5405_struct *fan_struct, uint8_t otg_level){
        ...
        mt_set_gpio_mode(GPIO27, 0);    /*mode 0 as gpio*/
        mt_set_gpio_dir(GPIO27, GPIO_DIR_OUT);  /*griffin:GPIO27(DPIR4); martell:GPIO200; get STAT pin status */
        mt_set_gpio_out(GPIO27, (otg_level == 0)? GPIO_OUT_ZERO : GPIO_OUT_ONE);           /**/
}
...
{	
	...
	fan_struct->ops->set_con6_val(fan_struct, 0x07, 0x0a);	/*1.25A, 4.42V --> 4.42V*/
