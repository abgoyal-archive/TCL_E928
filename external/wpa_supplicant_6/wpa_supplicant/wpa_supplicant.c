...
void wpa_supplicant_set_state(struct wpa_supplicant *wpa_s, wpa_states state)
{
	...
	{
		if(wpa_s->current_ssid == NULL) return;
		if(wpa_config_get(wpa_s->current_ssid,"eap")!=NULL)
		{
			if( os_strcmp(wpa_config_get(wpa_s->current_ssid,"eap"),"SIM")==0 || os_strcmp(wpa_config_get(wpa_s->current_ssid,"eap"),"AKA")==0 )
			{
