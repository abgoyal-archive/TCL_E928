    ...
    if (rcall != NULL) {
        ...
        switch (type) {
            ...
            case REMOTE_CALL_BUSY:
                ...
                break;
//MTK-START [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
            case REMOTE_CALL_HELD:
//MTK-END [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
                p = bufprint(p, end, "gsm hold " PHONE_PREFIX "%d\n", from_num);
                break;
//MTK-START [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
            case REMOTE_CALL_ACCEPTED:
//MTK-END [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
...
//MTK-START [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
    p  = bufprint(p, end, "sms pdu%d ",to_sim);
		 printf("[Emu] remote_call_set_sms_pdu %s\r\n ",p);
		 printf("[Emu] remote_call_set_sms_pdu %d\r\n ",to_sim);
                 ...
                 SmsPDU        pdu )
{
    ...
    if (call != NULL) {
//MTK-START [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
        if ( remote_call_set_sms_pdu( call, pdu,1) < 0 ) {
//MTK-END [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
            remote_call_free(call);
            return -1;
        }
...
//MTK-START [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
static RemoteCall
remote_call_alloc_gemini( RemoteCallType  type, int  to_port, int to_sim,int  from_port,int from_sim )
{
    RemoteCall  rcall    = calloc( sizeof(*rcall), 1 );
    int         from_num = remote_number_from_port(from_port);
    if (rcall != NULL) {
	 ...
	 rcall->to_sim  = to_sim;
        ...
        switch (type) {
            case REMOTE_CALL_DIAL:
                p = bufprint(p, end, "gsm call%d %d%d\n",to_sim, from_sim,from_num);
                break;

            case REMOTE_CALL_BUSY:
                p = bufprint(p, end, "gsm rejected %d%d\n",from_sim, from_num);
                break;
				
	     case REMOTE_CALL_HANGUP:
                p = bufprint(p, end, "gsm canceled %d%d\n", from_sim,from_num );
                break;

            case REMOTE_CALL_HELD:
                p = bufprint(p, end, "gsm held %d%d\n", from_sim,from_num);
                break;

            case REMOTE_CALL_ACCEPTED:
                p = bufprint(p, end, "gsm accepted %d%d\n", from_sim,from_num);
                break;
#if 0
	     case REMOTE_CALL_SMS:
		  p = bufprint(p, end, "sms send%d %d%d\n", to_sim,from_sim,from_num);
                break;
...
static RemoteCall
remote_call_generic_gemini( RemoteCallType  type, const char*  to_number, int  from_port ,int from_sim)
{
    int         to_port = remote_number_string_to_port(to_number+1);
    RemoteCall  call; 
    ...
    int to_sim = to_number[0] - '0';
    call = remote_call_alloc_gemini( type, to_port,to_sim, from_port ,from_sim);
    if (call == NULL) {
        return NULL;
    }
    remote_call_add( call, &_the_remote_calls );
...
int
remote_call_dial_gemini( const char*       number,
                  int               from,
                  RemoteResultFunc  result_func,
                  void*             result_opaque ,
                  int sim)
{
    RemoteCall   call = remote_call_generic_gemini( REMOTE_CALL_DIAL, number, from,sim );
...
void
remote_call_other_gemini( const char*  to_number, int  from_port, RemoteCallType  type, int from_sim)
{
    remote_call_generic_gemini( type, to_number, from_port,from_sim );
}

int
remote_call_sms_gemini( const char*   number,
                 int           from,
                 SmsPDU        pdu,
                 int from_sim)
{
    RemoteCall   call = remote_call_generic_gemini( REMOTE_CALL_SMS, number, from,from_sim );

    if (call == NULL)
        return -1;
    int to_sim = number[0] - '0';
    if (call != NULL) {
        if ( remote_call_set_sms_pdu( call, pdu, to_sim) < 0 ) {
            remote_call_free(call);
            return -1;
        }
    }
...
void
remote_call_cancel_gemini( const char*  to_number, int  from_port ,int from_sim, RemoteCallType  type)
{
    remote_call_generic_gemini( type, to_number, from_port ,from_sim);
}
//MTK-END [mtk80950] [ALPSXXXXXXXX] [111115] porting MTK-code to ICS
